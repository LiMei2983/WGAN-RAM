...
class Solver(object):
    def __init__(self, args, data_loader):
        ...
        self.WGANVGG = WGAN_VGG(input_size=args.patch_size if args.patch_n else 512)  # 512
    ...
    def train(self):
        ...
        param = self.get_parameter_number(self.WGANVGG)
        print('param=', param)
        for epoch in range(1, self.num_epochs):
            for iter_, (x, y) in enumerate(self.data_loader):
                ...
                g_loss = self.WGANVGG.g_loss(x, y)
                g_loss.backward()
                self.optimizer_g.step()
                train_losses.append([g_loss.item(), d_loss.item() - gp_loss.item(), gp_loss.item()])
                if total_iters % self.print_iters == 0:
                    print(
                        "STEP [{}], EPOCH [{}/{}], ITER [{}/{}], TIME [{:.1f}s]\nG_LOSS: {:.8f}, D_LOSS: {:.8f}".format(
                            total_iters, epoch, self.num_epochs, iter_ + 1, len(self.data_loader),
                            time.time() - start_time, g_loss.item(), d_loss.item()))
                # learning rate decay
